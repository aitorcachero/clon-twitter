...
export default function CardTwitter({ tweet, liked = false }) {
  ...
  useEffect(() => {
    const checkTweetLikes = async () => {
      const ARRAY_TWEETS = authUser?.arrayOfTweetLikes;
      console.group(ARRAY_TWEETS);
      const CHECK = await checkTweetLikedByMe(ARRAY_TWEETS, tweet.tweet_id);
      setLikedByMe(CHECK);
    };
    if (authUser) checkTweetLikes();
  }, []);
