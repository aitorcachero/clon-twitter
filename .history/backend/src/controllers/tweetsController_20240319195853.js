  ...
  const likesFunction = async (req, res) => {
    ...
    try {
      ...
      if (checkIsLiked.length === 0) {
        const like = tweetsModel().likeTweet(idAuth, user.id);
        res.send({
          status: 'ok',
          message: `Le has dado un like!`,
        });
      } else {
        const unlike = tweetsModel().unlikeTweet(idAuth, user.id);
        res.send({
          status: 'ok',
          message: `Le has quitado el like!`,
        });
      }
    } catch (error) {
      console.log(error);
