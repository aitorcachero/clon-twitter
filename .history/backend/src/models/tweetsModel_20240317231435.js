    ...
    try {
      if (!id) {
        const [result] = await db.query(
          'SELECT tweets.tweet_id, tweets.user_id, tweets.tweet_text, tweets.likes, tweets.retweets, tweets.comments, tweets.createdAt, users.id, users.username, users.name, users.surname FROM tweets, users ORDER BY tweets.createdAt ASC ;'
        );
        return result;
      } else {
        const [result] = await db.query(
