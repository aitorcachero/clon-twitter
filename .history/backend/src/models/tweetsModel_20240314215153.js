    ...
    try {
      if (!id) {
        const [result] = await db.query(
          'SELECT tweets.tweet_id, tweets.user_id, tweets.tweet_text, tweets.likes, tweets.retweets, tweets.comments, tweets.createdAt, users.id, users.username FROM tweets, users ;'
        );
        return result;
      } else {
        const [result] = await db.query(
